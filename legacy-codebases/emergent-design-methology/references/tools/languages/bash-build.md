...
- Use [`bash-logger`](https://github.com/GingerGraham/bash-logger) as the default Bash logging library. It provides reusable shell logging functions with standard severity levels and is a better default than inventing project-local `log_*` helpers each time. Use the `logger` command only for scripts that are explicitly integrated with syslog or journald. 
