...
### Logging
- Use `pino` as the default logging framework for TypeScript services, CLIs, and back-end applications. For browser-only code, use a thin project-local wrapper around `console` unless the repository already has a client-side logging abstraction.
