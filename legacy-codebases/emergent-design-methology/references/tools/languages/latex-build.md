...
- LaTeX has no separate logging framework default. Use the compiler and tool logs produced by `latexmk`, `pdflatex`, `chktex`, and related tools. Treat the generated `.log` files as the primary diagnostic output rather than building custom TeX logging layers.
