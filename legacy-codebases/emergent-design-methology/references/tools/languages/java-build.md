...
mvnd (Maven Daemon): If relevant use `mvnd` instead of `mvn`. It keeps a background daemon warm in the runtime environment, cutting compilation times down from seconds to milliseconds. Install from github downloads: https://github.com/apache/maven-mvnd/releases
...
- Use `SLF4J` as the default logging API. Pair it with `log4j` unless the project already standardises on another backend.
...
cmd_test() {
  ensure_em_dir
  mvnd test 2>&1 | grep -v "Download" | tee "$TEST_LOG"
}

cmd_check() {
  ensure_em_dir
  mvnd spotless:check checkstyle:check spotbugs:check 2>&1 | grep -v "Download" | tee "$CHECK_LOG"
}

cmd_doc() {
  ensure_em_dir
  mvnd javadoc:javadoc 2>&1  | grep -v "Download" | tee "$DOC_LOG"
}
```
