...
> [!note] Important
> Private templates are only supported on Docker Hub. `sbx` reuses your `sbx login` session to pull private images from Docker Hub. Other registries (such as GitHub Container Registry, ECR, or a self-hosted registry like Nexus) are pulled anonymously, so private images on those registries fail to pull.

For locally-built images or private images on registries that `sbx` can't authenticate against, save the image to a tar and load it directly into the sandbox runtime instead of pulling from a registry:
