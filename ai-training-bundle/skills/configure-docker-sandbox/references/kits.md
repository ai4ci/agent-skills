...
> [!note] Important
> Private kits are only supported on Docker Hub. `sbx` reuses your `sbx login` session to pull private artifacts from Docker Hub. Other registries are pulled anonymously, so private kits hosted on registries other than Docker Hub fail to pull.
